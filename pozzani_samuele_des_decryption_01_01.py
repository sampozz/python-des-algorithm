'''
Implementazione algoritmo di decodifica DES (Data Encryption Standard)
...
if __name__ == '__main__':
    ...
    # input
    m = input('Inserisci il messaggio da decodificare (valori esadecimali): ')
    k = input('Inserisci la chiave di crittografia (16 cifre esadecimali): ')

    # divisione di m in blocchi da 64 bit e decodifica
    n_blocks = len(m) // 16
    if len(m) % 16 != 0:
        n_blocks += 1
    c = ''
    for i in range(n_blocks):
        c = c + decryption(m[(i * 16):((i + 1) * 16)], k)

    print('Messaggio decodificato:', c)
